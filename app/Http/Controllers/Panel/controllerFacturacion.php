...
class controllerFacturacion extends Controller
{
    ...
    public function handleGetPedidos($sector){
        return DB::select(<<<EOF
            select * from (
                select SUM(REPORTE_OV.total_USD) as Total,REPORTE_OV.Nombre_Cliente,REPORTE_OV.Sector,REPORTE_OV.Sucursal, MONTH(REPORTE_OV.Fecha_Entrega2) as Moth, YEAR(REPORTE_OV.Fecha_Entrega2) as 'Year'  
                ...
                Group By REPORTE_OV.Nombre_Cliente,REPORTE_OV.Sector,REPORTE_OV.Sucursal,MONTH(REPORTE_OV.Fecha_Entrega2),YEAR(REPORTE_OV.Fecha_Entrega2)      
            ) T0 where Total>=1
    ...
    public function handleGetPedidoDetalle(Request $request){
        return DB::select(<<<EOF
            select T0.Nombre_Cliente,T0.Vendedor,T0.Descricion,T0.cod_ventas,T0.Nombre_Fabricante,T0.Cantidad,total_USD
    ...
    public function handleGetOportunidadesMes(Request $request){
        return DB::select(<<<EOF
            Select T0.Cliente,T0.Sector,T0.Sucursal,SUM(T0.TotalUSD) as Total,MONTH(T0.FechaEstimadaCierre) as Mes,YEAR(T0.FechaEstimadaCierre) as Año
            ...
            and T0.Probabilidad like '70% o mayor'
            Group By T0.Cliente,T0.Sector,T0.Sucursal,MONTH(T0.FechaEstimadaCierre),YEAR(T0.FechaEstimadaCierre)
            order By T0.Sucursal
        EOF);
    }
    public function handleGetOportunidadesAll(Request $request){
        return DB::select(<<<EOF
            ...
            and T0.Sector='$request->Sector' and (T0.EstadoOportunidad ='Propuesta' or T0.EstadoOportunidad ='Negociacion')
            and T0.Probabilidad like '70% o mayor'
            Group By T0.Cliente,T0.Sector,T0.Sucursal,MONTH(T0.FechaEstimadaCierre),YEAR(T0.FechaEstimadaCierre)
            order By T0.Sucursal
        EOF);
    ...
    public function handleGetOportunidadesMeses($sector){
        return DB::select(<<<EOF
            ...
            From [192.168.10.31].H2_Levcorp.dbo.RPT_OPORTUNIDADES T0
            where YEAR(T0.FechaEstimadaCierre) >= YEAR(GETDATE()) 
            and T0.Sector='$sector' and (T0.EstadoOportunidad ='Propuesta' or T0.EstadoOportunidad ='Negociacion')
            and T0.Probabilidad like '70% o mayor'
    ...
    public function handleGetOportunidadesAños($sector){
        return DB::select(<<<EOF
            ...
            where YEAR(T0.FechaEstimadaCierre) >= YEAR(GETDATE()) 
            and T0.Sector='$sector' and (T0.EstadoOportunidad ='Propuesta' or T0.EstadoOportunidad ='Negociacion')
            and CONVERT(varchar(150),T0.Probabilidad) like '70% o mayor'
