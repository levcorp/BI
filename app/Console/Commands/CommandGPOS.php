...
class CommandGPOS extends Command
{
    ...
    public function handle(){
        ...
        if(GPOS::whereDate('DocDate','>=',$lastSunday)->whereDate('DocDate','<=',$nextSaturday)->where('ShipFromDistributorDUNS+4','=','LARCOS000')->count()>0){
            ...
            Storage::disk('EDIftp')->put('\LaPaz_'.$lastSunday->format('Ymd').'a'.$nextSaturday->format('Ymd').'.txt', $gpos->text_date('LARCOS000','0000863151',$lastSunday,$nextSaturday));
        }
        if(GPOS::whereDate('DocDate','>=',$lastSunday)->whereDate('DocDate','<=',$nextSaturday)->where('ShipFromDistributorDUNS+4','=','LARCOS001')->count()>0){
            ...
            Storage::disk('EDIftp')->put('\SantaCruz_'.$lastSunday->format('Ymd').'a'.$nextSaturday->format('Ymd').'.txt', $gpos->text_date('LARCOS001','0000863153',$lastSunday,$nextSaturday));
        }
        if(GPOS::whereDate('DocDate','>=',$lastSunday)->whereDate('DocDate','<=',$nextSaturday)->where('ShipFromDistributorDUNS+4','=','LARCOS002')->count()>0){
            ...
            Storage::disk('EDIftp')->put('\Cochabamba_'.$lastSunday->format('Ymd').'a'.$nextSaturday->format('Ymd').'.txt', $gpos->text_date('LARCOS002','0000863152',$lastSunday,$nextSaturday));
        }
